...
const CalendarList = (props, ref) => {
    ...
    /** ScrollView props */
    horizontal = false, pagingEnabled, scrollEnabled = true, nestedScrollEnabled = true, scrollsToTop = false, keyExtractor = defaultKeyExtractor, keyboardShouldPersistTaps, onScrollBeginDrag, onScrollEndDrag, onMomentumScrollBegin, onMomentumScrollEnd, 
    /** FlatList props */
    onEndReachedThreshold, onEndReached } = props;
    const calendarProps = useMemoCompare(extractCalendarProps(props), (oldVal, newVal) => {
        return !!oldVal && Object.keys(newVal).every((key) => oldVal[key] === newVal[key]);
    });
    ...
    const renderItem = useCallback(({ item }) => {
        ...
        return (<CalendarListItem {...calendarProps} testID={testId} markedDates={markedDates} item={item} style={calendarStyle} 
