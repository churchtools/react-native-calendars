...
export default class Agenda extends Component<AgendaProps, State> {
  ...
  componentDidUpdate(prevProps: AgendaProps, prevState: State) {
    ...
    static getDerivedStateFromProps(nextProps: AgendaProps, previousState: State) {
        if (nextProps.items) {
            const state: { firstReservationLoad: boolean; markedDates: Record<string, MarkingProps> } = {
                firstReservationLoad: false,
                markedDates: {}
            };
        ...
        return (
            <View testID={testID} onLayout={this.onLayout} style={[style, this.style.container]}>
                ...
                <Animated.View style={headerStyle}>
                    <Animated.View
                        style={[this.style.animatedContainer, {transform: [{translateY: contentTranslate}]}]}>
